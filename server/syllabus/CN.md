...
### 3. TCP 3-Way Handshake & Connection Teardown (Medium)
...
### 4. Socket Programming Basics (Medium)
...
* **System Design / Interview Classic:** What happens behind the scenes at every layer of the network stack when you type `https://google.com` into a browser and press Enter?
* **LeetCode / Custom Simulation:** Implement a simplified, multi-threaded TCP socket listener loop in Python that safely processes and logs concurrent network packets without blocking the main worker thread.
...
## Sub-Topics & Concepts
### 1. IPv4 Addressing & Subnetting (Medium)
...
### 3. Routing Protocols (BGP, OSPF) (Medium)
...
## High-Yield Reference Questions
* **Standard Placement Classic:** Given an enterprise network block of `192.168.1.0/24`, logically slice the architecture to support 4 distinct departments requiring at least 30 host IPs each, and define the exact subnet masks.
* **Architectural Trade-offs:** Explain how BGP loop prevention rules work using the `AS_PATH` attribute, and outline what happens to global traffic when an ISP misconfigures its route tables.
...
## High-Yield Reference Questions
* **System Optimization:** How does HTTP/2 multiplexing work over a single TCP connection, and how does HTTP/3 resolve the TCP Head-of-Line blocking issue when a packet is dropped?
* **Edge Case Analysis:** What is a DNS amplification attack, and how do attackers manipulate open recursive resolvers and UDP packet size characteristics to overwhelm an enterprise backend?
...
## Sub-Topics & Concepts
### 1. SSL/TLS 1.2 & 1.3 Handshakes (Hard)
...
## High-Yield Reference Questions
* **Crypto Deep-Dive:** Walk through the complete cryptographic sequence of a TLS 1.3 handshake. Explain how a shared secret is established without ever passing it across the network wire.
* **System Design / Scalability:** Design a high-availability infrastructure layout for a Web SaaS platform that can dynamically absorb and filter a massive multi-gigabit SYN Flood attack without dropping legitimate user sessions.