...
### 2. Process Scheduling Algorithms (Medium)
...
### 4. Thread Synchronization (Medium)
...
* **System Design / Interview Classic:** Walk through the complete lifecycle of a process across its structural states (`New`, `Ready`, `Running`, `Waiting`, `Terminated`). Detail exactly how a hardware I/O interrupt alters its PCB placement within the kernel queues.
* **Low-Level Analysis:** Explain why context-switching between two threads within the same process is significantly faster than context-switching between two entirely distinct processes.
...
### 2. Page Replacement Algorithms (Medium)
...
### 4. Memory Allocation Strategies (Medium)
...
## High-Yield Reference Questions
* **System Trace Challenge:** Given a 32-bit virtual address format with a 4KB page configuration, walk through the exact mathematical bit-shifting steps the MMU takes to isolate the page number and page offset.
* **Production Debugging:** You observe a cloud server's CPU usage spike to 100% while its application throughput drops to zero. Explain how you would verify if this is caused by a memory leak triggering continuous system Thrashing.
...
### 2. Deadlock Detection & Avoidance (Hard)
...
### 3. Reader-Writer Problem (Medium)
...
## High-Yield Reference Questions
* **Concurrency Implementation:** Write a pseudocode solution for the Bounded-Buffer (Producer-Consumer) problem using Semaphores, highlighting how you arrange the locks to prevent deadlocks.
* **Architectural Trade-offs:** Explain the concept of Priority Inversion and detail how the Priority Inheritance Protocol solves it when a low-priority process holds a lock needed by a high-priority process.
...
## Sub-Topics & Concepts
### 1. File Allocation Methods (Medium)
...
### 2. Disk Scheduling Algorithms (Medium)
...
### 3. File Permissions & Access Control (Easy)
...
## High-Yield Reference Questions
* **File System Deep-Dive:** Walk through how a traditional UNIX file system parses an Inode structure to locate and read blocks from a massive 4GB file on disk.
* **Hardware Orchestration:** Explain how Direct Memory Access (DMA) enhances system performance by freeing the CPU from managing data transfers between I/O devices and main memory.